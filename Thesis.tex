...
To Massimo and Giorgia, for guiding me through this final step of the master’s program, and for introducing me to this wonderful city, Ascoli Piceno.
...
A Massimo y Giorgia, por haberme acompañado en este último escalón del máster, y por haberme presentado esta hermosa ciudad que es Ascoli Piceno.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.4\textwidth]{Images/pw_attributes.png}
    \captionsetup{font=footnotesize}
...
\section{Current Approaches and Methodological Considerations}

\indent Tourism research has used many different methods to study and understand the attributes that shape tourist behavior and destination choice. However, even with the wide range of models and analytical tools available, most approaches remain somewhat fragmented, narrow in scope, or not fully adapted to the complexity of modern tourism. In Chapter 2, some of these actual methodologies are described, such as qualitative and quantitative methods, destination image models, segmentation techniques, discrete choice and conjoint analyses, and studies that deal with hypothetical bias and tourist heterogeneity. Together, these areas form the theoretical and methodological foundation of this project, while also showing the main limitations that motivate the search for a more efficient and integrated framework for defining tourism attributes.

\indent In tourism research, methods can generally be divided into qualitative and quantitative. Qualitative research provides depth and context, helping to explore the meanings, motivations, and experiences that shape tourist behavior. It focuses on open-ended questions and interpretation, allowing new ideas and relationships to emerge . However, because of its subjective nature and smaller samples, qualitative work can be difficult to replicate or compare across cases. Quantitative research, on the other hand, offers structure, consistency, and the possibility of generalizing results. Yet, by relying on predefined variables and rigid designs, it often simplifies complex and changing phenomena into limited indicators. Both approaches therefore have strengths and weaknesses, and neither alone can fully capture the cognitive, emotional, and contextual dimensions of tourism attributes \cite{creswelletal, finnetal, veal}.

\indent Among quantitative models, destination image studies, such as Baloglu and McCleary (1999), have been highly influential in explaining how tourists form perceptions of destinations. Their framework combines cognitive (knowledge-based) and affective (emotional) components and shows how these jointly shape overall image. While this was an important step forward, the model still depends on fixed sets of attributes and assumes simple causal relationships. In addition, most destination image studies rely on self-reported survey data, which can be influenced by response styles and hypothetical situations \cite{destinationimg}.

\indent Similar issues appear in segmentation research, where methods like k-means clustering are used to group tourists according to their motivations or attitudes. Pesonen and Honkanen (2014) show that these results can be affected by response-style effects, such as the tendency to give consistently high or low scores, creating artificial clusters that do not reflect real differences in behavior. This highlights a deeper problem in quantitative tourism research: the assumption that numbers collected through rating scales always represent genuine preferences. Without correcting for such biases or using complementary methods, segmentation studies can lead to unreliable or misleading conclusions \cite{clustering}.

\indent Discrete choice experiments (DCEs) and conjoint analysis are more advanced methods used to measure how tourists value different destination or product attributes. Based on random utility theory, they help estimate the relative importance of each attribute and the trade-offs travelers are willing to make. These techniques simulate realistic decision-making processes and provide valuable insights into preference structures. Still, they are affected by design complexity and hypothetical bias (the difference between what people say they would choose in a survey and what they actually do in real life). Despite various strategies to reduce this bias, such as calibration or “cheap-talk” scripts, it remains a persistent challenge in stated-choice studies \cite{discretechoice, conjointaltin, hypotheticalbias}.

\indent Finally, new models have begun to address the heterogeneity and interdependence of tourist decisions. For example, Wu, Zhang, and Fujiwara (2011) combine latent class and nested logit models to jointly analyze choices of destination and travel party. Their approach recognizes that travel decisions are interconnected and vary across individuals. While this represents progress toward a more realistic understanding of tourist behavior, these models still rely on predefined variables and rational assumptions that may not fully capture the emotional or situational factors behind decision-making \cite{touristheterogenity}.

\indent In conclusion, the existing literature offers many valuable methods for identifying and studying tourism attributes, but most remain limited in different ways. Qualitative studies provide depth but lack comparability; quantitative models bring precision but lose context; and hybrid methods like DCEs and conjoint analysis aim for realism but still face issues of bias and rigidity. Moreover, many frameworks view attributes as fixed and separate rather than as part of an evolving and interactive system.

\indent This project builds on these insights to propose a methodological approach that combines the strengths of different methods while addressing their weaknesses. By linking the precision of quantitative tools with the flexibility and depth of qualitative inquiry, the study aims to develop a more practical and adaptive way to define and analyze tourism attributes—one that reflects the real complexity of tourism behavior and experiences today.
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=1\textwidth]{Material/2. Interviews & Surveys/1. Qualitative Interview/Qualitative Interview Questions.png}}
    \captionsetup{font=footnotesize}
...
\indent IN TF-IDF ADD SOME WORDS ABOUT THE PAPERS Hu \& Liu 2004; Pang \& Lee 2008 PARA SUMAR POR LA DEFINICION DE GRAMS.
