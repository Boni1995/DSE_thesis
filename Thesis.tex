...
\indent One of the datasets used in this analysis was related to Airbnb reviews in different Italian cities. This was obtained from Inside Airbnb, where quarterly data for the last year for different regions is available for free download \cite{airbnbdata}. The Italian cities included in this analysis were Bergamo, Bologna, Florence, Milan, Naples, Puglia, Rome, Sicily, Trentino, and Venice, and all of the individual datasets were concatenated into one single dataset. A total of 7,141,844 records were retrieved, including listing\_id, id, date, reviewer\_id, reviewer\_name, comments and locations. To optimize workload and time, a new column was created with the count of words used in the comments column, and the data was imited to the upper queartile (75\%) of this new column. This ended up with a dataset of 5,383,460 records. Following with the data preparation, a language detector was implemented as comments had a mix of lanugages. For this, the Compact Language Detector 2 was used, which probabilistically detects over 80 languages in Unicode UTF-8 text \cite{cld2}. To reduce data size, only the 2,800,779 english reviews were retrieved (see Figure~\ref{fig:cld2}). Finally, only a 15\% of the english reviews was used for the analysis, aiming to optimize and reduce the running time of the natural language programming models implemented. 
...
The TF–IDF scores were then aggregated across all documents to obtain a global measure of each term’s importance. To visualize the most frequent terms in the corpus, a word cloud was generated using the WordCloud library, with word size proportional to cumulative TF–IDF scores. Additionally, the top 20 terms were identified and displayed in tabular form, ranked by their total TF–IDF score.

\subsection{Booking.com}

\indent The Booking.com Reviews Dataset is a comprehensive collection of hotel reviews and ratings gathered from the popular travel booking website, Booking.com. This dataset provides valuable insights into the experiences and opinions of customers who have stayed at various hotels across different locations \cite{bookingdata}. After importing the dataset, reviews containing the text “There are no comments available for this review” were removed, leaving a total of 18,988 valid records for analysis. Each review was then tokenized using the NLTK word\_tokenize function, with all tokens converted to lowercase to ensure uniformity. As in the Airbnb pipeline, stopwords (from NLTK’s English stopword list) and punctuation marks (from Python’s string library) were filtered out, producing a set of cleaned tokens. Subsequently, the tokens were lemmatized using the spaCy language model. This step reduced words to their base form, thereby consolidating morphological variants. A final cleaning step removed generic terms such as “review” and “comment” that added little semantic value. After all this preprocessing steps, the dataset was splited into positive and negative reviews based on the rating score:
...
\indent For each subset (positive and negative reviews), a TF–IDF vectorizer was applied to transform the lemmatized text into a numerical representation. The vectorizer parameters were configured as follows:
...
\section{Explanatory Survey for Attribute Validation}

\indent Further in the analysis, an explanatory survey (see Appendix D) was conducted, through Microsoft Forms, to capture travelers’ perceptions, decision-making processes, and experiences related to tourism. The questions were were written in English and Spanish. It was sent to 25 respondents, with no additional details. After reaching the 25 responses the survey was closed. This worked as a bridge between the qualitative interviews and the final quantitative survey, serving both to test the clarity of questions and to generate additional insights for attribute refinement. The survey consisted of a combination of open-ended and structured questions. Open-ended questions invited respondents to narrate personal travel experiences (e.g., bad trip story, representative trip, vacation story) and to provide advice to their “past self” in terms of decision-making. Scenario-based questions (scene choices) were included to explore how travelers evaluate different destination settings. Structured questions addressed decision-making steps, missing factors in information search, levels of information saturation, and perceptions of pricing. For this last topic, a Van Westendorp’s Price Sensitivity Model's question was included to estimate acceptable price ranges for a touristic package. This model provides a more comprehensive, multi-question model that indirectly measures willingness to pay instead of directly posing the question to potential buyers. Rather than asking potential buyers to identify a single price point, the Van Westendorp model helps assess a range of prices instead of just one \cite{vanwestern}.

\indent For the analysis, Google Colab was used as it provides 
...
\indent When it comes to the Booking.com dataset, the unigram analysis of positive reviews revealed that the most frequent terms were, once again, related to both the location of the property and the services offered by the accommodation. As shown in Figure~\ref{fig:bookingpword}, compared to the Airbnb reviews, a greater number of terms referred to specific services, such as ``breakfast'', ``reception'', ``garden'', and ``bed'', among others. In addition, new location-related terms appeared, including ``metro'' and ``restaurant.''
...
\indent In contrast, the unigram analysis of negative reviews (see Figure~\ref{fig:bookingnword}) indicated that, although ``location'' remained a highly frequent term, a larger share of the most common words referred to accommodation services, such as ``shower'', ``bathroom'', ``bed'', and ``staff'', among others.
...
\indent Based on the insights obtained from this text analysis across both datasets, the list of attributes was further refined and improved as follows:
...
\begin{itemize}
    \item Age range: 13 respondents between 25 and 35 years old, 8 respondnets 60 or older, 3 respondents between 35 and 39, and 1 respondents younger than 25 years old.
    \item Gender: 16 females and 9 males.
    \item Region: 17 respondents from South America, 5 respondents from Italy, and 3 respondents from Europe (excluding Italy).
    \item Professional status: most respondents (11) are employed, 5 are self-employed, 5 are retired, and 4 are students.
    \item Trip organization: in general younger respondents tend to organize trips with other people, while older people tend to organize trips themselves.
    \item Travelling purpose: in general respondents travel to explore new cities or for relaxation.
    \item Type of traveler: respondents tend to travel more 
\end{itemize}
...
\includepdf[
  pages=1,
  pagecommand={\section*{C. Qualitative Interview Notes}}
...
\includepdf[
  ...
  pagecommand={
    \vspace*{-2.5cm}
    \section*{D. Explanatory Survey}
  }
]{Material/2. Interviews/2. Intermediate Survey/Explanatory Survey.pdf}
